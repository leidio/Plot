...
Rules when the user requests a change:
  ...
  2. Wrap the code in a single fenced code block with language tag: \`\`\`jsx ... \`\`\`
  3. After the code block, write one sentence describing what changed — nothing more
  4. ONLY return a fenced code block if you are returning a complete file rewrite. Never use fenced code blocks for snippets, examples, or answers to questions.

Rules when the user asks a question (no code change needed):
  Answer directly in plain text. Do NOT use fenced code blocks. Use inline backticks only for short values like class names or variable names.`
  ...
  try {
    ...
    const code        = codeMatch && codeMatch[1].split('\n').length > 10 ? codeMatch[1].trimEnd() : null
...
.proposed-bar{padding:8px 16px;background:var(--green-dim);border-bottom:1px solid var(--green-border);display:flex;align-items:flex-start;gap:10px;flex-shrink:0;flex-wrap:wrap}
...
.code-scroll{height:100%;overflow:auto;padding:16px 0}
.code-table{border-collapse:collapse;width:100%;font-family:var(--mono);font-size:11.5px;line-height:1.7}
.ln{width:1px;padding:0 14px 0 20px;text-align:right;color:var(--dim);user-select:none;white-space:nowrap;border-right:1px solid var(--border-sub)}
.lc{padding:0 20px;color:#d4d4d8;white-space:pre}
  ...
  <main>
    ...
    <div class="code-panel">
      ...
      <div class="proposed-bar hidden" id="propBar">
        <div class="pdot"></div>
        <span class="plabel2" id="propLabel" style="flex:1;min-width:180px">Change ready to apply</span>
        <div class="pbars">
...
function showCode(c) {
  ...
  el.style.display = ''
  const lines = c.split('\\n')
  el.innerHTML = '<table class="code-table"><tbody>' +
    lines.map(function(line, i) {
      return '<tr><td class="ln">' + (i + 1) + '</td><td class="lc">' + esc(line) + '</td></tr>'
    }).join('') +
    '</tbody></table>'
}
