...
const MovementPreviewModal = ({ movement, onClose, onViewFullPage }) => {
  ...
  return (
    ...
    >
      <div 
        className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-8 space-y-6">
          <div className="flex justify-between items-start">
            <h2 className={`text-center text-3xl leading-tight font-semibold pr-8 ${isDark ? 'text-gray-100' : ''}`}>{movement.name || 'Untitled Movement'}</h2>
            <button onClick={onClose} className={isDark ? 'text-gray-400 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}>
              <X className="w-6 h-6" />
            </button>
          </div>

          <p className={`text-lg leading-tight ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{movement.description || 'No description'}</p>

          <div className={`${isDark ? '' : ''} rounded-lg text-lg ${isDark ? 'text-gray-300' : 'text-gray-700'} space-y-2`}>
            <div className="flex items-center space-x-2">
              <MapPin className={`w-4 h-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
              <span className={`leading-tight ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{movement.city}, {movement.state}</span>
            </div>
          </div>
